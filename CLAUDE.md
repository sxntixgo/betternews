...
- `app/api_tokens.py` — per-device bearer tokens, SHA-256 hashed (high-entropy random needs no slow KDF, and a slow one would run on every request). `@api_auth` never falls back to the session, and `auth._is_api_request()` exempts `/api/` from the session guards — without it the app-wide `before_request` answered every API call with a 302 to `/login`, valid token or not.
- **The API deliberately does not cover settings, admin or insights** (37 of the 70 HTML routes). They are panels visited monthly from a desktop; porting them to JSON and rebuilding them in a client is weeks of work for nothing. See `docs/api-and-spa-plan.md` B.4.
