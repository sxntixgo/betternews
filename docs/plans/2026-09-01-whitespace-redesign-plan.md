...
## Environment Gate — read before dispatching Task 1
...
### Task 1: Make the web toolchain runnable
...
**Interfaces:**
- Consumes: nothing.
- Produces: a `node` on `PATH`, Linux `rolldown` bindings, and Playwright `chromium` + `webkit` browsers. Every later task depends on all three.

- [ ] **Step 1: Put node on PATH**

```bash
NODE=$(find /vscode/vscode-server/bin/linux-x64 -maxdepth 2 -name node -type f | head -1)
mkdir -p /tmp/nodebin && ln -sf "$NODE" /tmp/nodebin/node
export PATH="/tmp/nodebin:$PATH"
node --version
```

Expected: `v22.22.1` or newer.

- [ ] **Step 2: Reinstall web dependencies for Linux**

```bash
cd /workspaces/repositories/rss-reader/web
rm -rf node_modules package-lock.json
node /tmp/nodebin/../nodebin/node "$(command -v npm 2>/dev/null || echo /usr/lib/node_modules/npm/bin/npm-cli.js)" install
```

If `npm` is not resolvable, use the npm bundled with the VS Code node:

```bash
NPM=$(dirname $(find /vscode/vscode-server/bin/linux-x64 -maxdepth 2 -name node -type f | head -1))/../lib/node_modules/npm/bin/npm-cli.js
node "$NPM" install
```

Expected: `@rolldown/binding-linux-x64-gnu` appears in `node_modules/@rolldown/`.

- [ ] **Step 3: Verify the build now runs**

Run: `node ./node_modules/.bin/vite build`
Expected: `✓ built in …`, and `web/dist/assets/` contains a hashed `index-*.js`.

- [ ] **Step 4: Install Playwright browsers**

```bash
node ./node_modules/.bin/playwright install --with-deps chromium webkit
```

Expected: `~/.cache/ms-playwright/` contains `chromium-*` and `webkit-*`.

- [ ] **Step 5: Verify the existing suite is green before any redesign work**

Run: `node ./node_modules/.bin/playwright test --project=desktop`
Expected: all tests pass. **If any fail now, stop and report — the baseline must be green before Task 3 changes every colour in the app.**

- [ ] **Step 6: Commit**

```bash
cd /workspaces/repositories/rss-reader
git checkout -b feat/whitespace-redesign
git add web/package-lock.json
git commit -m "chore: reinstall web deps for linux and add playwright browsers"
```
