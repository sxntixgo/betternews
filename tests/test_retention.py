...
def test_retention_form_renders_preview(client, app):
    from app.db import get_db_direct
    with app.app_context():
        db = get_db_direct()
        add_article(db, add_feed(db), created_at=_old(90))
        db.close()
    data = client.get("/settings/retention").get_data(as_text=True)
    assert "Keep articles for" in data
    assert "Would delete <strong>1</strong> of 1" in data


def test_retention_form_when_disabled(client, app):
    from app.db import get_db_direct, set_setting
    with app.app_context():
        db = get_db_direct()
        set_setting(db, retention.SETTING_DAYS, "0")
        db.commit()
        db.close()
    assert "Pruning disabled" in client.get("/settings/retention").get_data(as_text=True)


def test_retention_save_persists_days_and_confirmation(client, app):
    r = client.post("/settings/retention",
                    data={"retention_days": "30", "retention_confirmed": "1"})
    assert r.status_code == 200
    from app.db import get_db_direct
    with app.app_context():
        db = get_db_direct()
        assert retention.retention_days(db) == 30
        assert retention.is_confirmed(db) is True
        db.close()


def test_retention_save_can_unconfirm(client, app):
    client.post("/settings/retention",
                data={"retention_days": "15", "retention_confirmed": "1"})
    client.post("/settings/retention", data={"retention_days": "15"})
    from app.db import get_db_direct
    with app.app_context():
        db = get_db_direct()
        assert retention.is_confirmed(db) is False
        db.close()


@pytest.mark.parametrize("raw", ["", "abc", "-1"])
def test_retention_save_rejects_bad_days(client, raw):
    assert client.post("/settings/retention",
                       data={"retention_days": raw}).status_code == 400


def test_prune_now_requires_confirmation(client):
    r = client.post("/settings/retention/prune")
    assert r.status_code == 400
    assert b"confirm" in r.data


def test_prune_now_deletes_when_confirmed(client, app):
    from app.db import get_db_direct
    with app.app_context():
        db = get_db_direct()
        add_article(db, add_feed(db), created_at=_old(90))
        retention.set_confirmed(db, True)
        db.commit()
        db.close()
    r = client.post("/settings/retention/prune")
    assert r.status_code == 200
    assert b"Pruned 1" in r.data
    with app.app_context():
        db = get_db_direct()
        assert db.execute(text("SELECT COUNT(*) FROM articles")).scalar() == 0
        db.close()


def test_clear_read_route_for_one_user(client, app):
    from app.db import get_db_direct
    from app.repo.articles import mark_read
    with app.app_context():
        db = get_db_direct()
        uid = _uid(db)
        mark_read(db, uid, add_article(db, add_feed(db)))
        db.commit()
        db.close()
    r = client.post("/settings/retention/clear-read", data={"user_id": str(uid)})
    assert r.status_code == 200
    assert b"Removed 1" in r.data


def test_clear_read_route_all_users(client, app):
    from app.db import get_db_direct
    from app.repo.articles import mark_read
    with app.app_context():
        db = get_db_direct()
        a = _uid(db)
        b = add_user(db, username="second", role="user")
        fid = add_feed(db)
        aid = add_article(db, fid)
        mark_read(db, a, aid)
        mark_read(db, b, aid)
        db.commit()
        db.close()
    r = client.post("/settings/retention/clear-read", data={"all_users": "1"})
    assert b"Removed 2" in r.data


def test_clear_read_route_requires_a_selection(client):
    r = client.post("/settings/retention/clear-read", data={})
    assert r.status_code == 400
    assert b"select at least one user" in r.data
